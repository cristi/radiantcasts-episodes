...
class PaperclippedExtension < Radiant::Extension
  ...
  def activate
    ...
    %w{page}.each do |view|
      admin.send(view).edit.add :form, "/admin/bucket/show_bucket_link", :before => "edit_page_parts"
      admin.send(view).edit.add :main, "/admin/bucket/assets_bucket", :after => "edit_buttons"
